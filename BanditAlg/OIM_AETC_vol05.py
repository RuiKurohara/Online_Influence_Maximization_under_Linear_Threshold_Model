...
class OIM_AETC_Algorithm:
    def __init__(self, G, EwTrue, seedSize, oracle, iterationTime, a,b,c):
        # アルゴリズムのパラメータ初期化
        self.G = G
    ...
    def decide(self):
        if self.iterCounter < self.G.number_of_nodes() * self.b:
            ...
            if self.iterCounter == self.G.number_of_nodes() * self.b - 1:#初期実行最後
                self.update_end_explore(self,self.a)
            ...
            # 動的に探索フェーズを終了する条件
            if self.pulls[uToLearning] == self.c - 1:
                self.end_explore[uToLearning] = True
            
        if list(self.end_explore.values()).count(False) == 0:
            self.explore_phase = False  # 探索フェーズ終了
            self.endCount = self.iterCounter
            # 活用フェーズに移行'
            
        if not self.explore_phase and self.isFirst_commit:
    ...
    def update_end_explore(self, n_percent):
        # 1. estimated_mean_stの全要素を配列に変換
        all_values = np.array(list(self.estimated_mean_st.values()))
        
        # 2. n%に対応する閾値を計算
        threshold = np.percentile(all_values, n_percent)
        
        # 3. estimated_mean_stの値が閾値以下の要素に対応するend_exploreをTrueにする
        for uToLearning, value in self.estimated_mean_st.items():
            if value <= threshold:
                self.end_explore[uToLearning] = True
