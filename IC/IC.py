...
def checkIC(G, Ew, eps = 1e-4):#使わない
    for u in G:
        in_edges = G.in_edges([u], data=True)
        total = 0
        for (v1, v2, edata) in in_edges:
            total += Ew[(v1, v2)]*G[v1][v2]['weight'] #使用する場合ICに
        if total >= 1 + eps:
            return 'For node %s IC property is incorrect. Sum equals to %s' %(u, total)
    return True

def runIC(G, S, Ew, lv ,p = .01):
    '''
    ...
    while len(Sj):  # while we have newly activated nodes
        ...
        for u in Sj:
            for v in G[u]:  # In G，Sj u's out edge to v。
                if v not in T:
                    if lv[v] <= 1 - (1-p)**Ew[(u,v)]: # if at least one of edges propagate influence
...
def runIC_NodeFeedback(G, S, Ew ,lv, p=.01):
    '''
    Input: G -- networkx directed graph
    S -- initial seed set of nodes
    Ew -- influence weights of edges
...
def avgIC(G, S, Ew, lv, iterations):
    avgSize = 0
    progress = 1
